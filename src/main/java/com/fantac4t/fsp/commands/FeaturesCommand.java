...
public final class FeaturesCommand {
    ...
    private static void sendCommands(CommandSourceStack src) {
        ...
        send(src, cmd("/live link <url>",              "set your stream link (max 256 characters) — a twitch.tv URL is auto-detected for Twitch auto-live"));
        send(src, cmd("/live link",                    "show your current stream link"));
        send(src, cmd("/live link clear",              "remove your stream link"));
        send(src, note("Twitch auto-live (going live on Twitch flips your in-game status automatically) only works if the server admin has set twitch.clientId and twitch.clientSecret in config.toml. Without it, use /live to toggle manually."));
        ...
        send(src, cmd("/tag set <player> <text>",      "set a player's nametag suffix (max 48 chars) — works for offline players too"));
        send(src, cmd("/tag get <player>",             "view a player's current nametag suffix — works for offline players too"));
        send(src, cmd("/tag remove <player>",          "clear a player's nametag suffix — works for offline players too"));
        send(src, cmd("/tag list",                     "list every player who currently has a nametag suffix set"));
        send(src, cmd("/fsp reload",                   "hot-reload config.toml without restarting the server"));
    ...
    private static void sendPlaceholders(CommandSourceStack src) {
        ...
        send(src, ph("%fsp:vc_status%",        "the player's current voice chat state icon — deafened, disconnected, speaking or in-group (mute is client-only, so a server cannot see it)"));
